...
import {
  ...
  Package, TrendingUp, AlertTriangle, Briefcase, Clock,
...
export function RetailerDrillDown({ mission, region, simMultipliers, onBack }: RetailerDrillDownProps) {
  ...
  const handleGenerateScript = (customer: CustomerProfile) => {
    ...
    setTimeout(() => {
      setScriptGenerated(
        `ACCOUNT OUTREACH SCRIPT — ${customer.name} — ${new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}\n\n` +
        `OPENING:\n"Good [morning/afternoon], ${customer.name}. This is [Account Manager Name] from ${selectedRetailer?.name}, calling on behalf of your dedicated Amcor account team. I'm reaching out because we've identified a very specific opportunity that I believe is well-timed for your business."\n\n` +
        `PERSONALISATION HOOK:\nReference: ${customer.currentFormat} (${customer.currentSpec}) — expansion value: ${customer.expansionValue}\n\n` +
        `VALUE PROPOSITION:\n"Your current ${customer.currentFormat} programme is performing well — our team has identified a ${customer.expansionValue} expansion opportunity that, combined with our current sustainable-format innovation pipeline, creates a compelling commercial case that may not be available beyond next quarter."\n\n` +
        `MISSION CONTEXT:\n${mission.title}\n"${mission.tagline}"\n\n` +
        `RECOMMENDED NEXT STEP:\n${customer.recommendedAction}\n\n` +
        `CONTACT PREFERENCE:\nBest window: ${customer.contactWindow}\nPreferred channel: ${customer.preferredChannel}\n\n` +
        `OBJECTION HANDLING:\n• If 'not the right time': "I completely understand. The expansion pricing I mentioned is time-limited — our allocation window closes ${closeDate(mission.therefore.timelineDaysFromNow.close)}. Would it help if I arranged a no-obligation technical review at a time that suits?"\n• If 'happy with current supplier mix': "That speaks to a well-managed supply chain. This programme is specifically designed for accounts already performing well — it's about de-risking supply and improving the sustainability profile of your current packaging mix."\n\nCLOSING:\n"Given your tenure of ${customer.tenure} with Amcor, you'll have first access. I'll follow this up with a personalised proposal document — what email address should I use?"\n\n— Generated by Lumina Intelligence Engine v3.4`
      );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <ChevronRight className="w-3.5 h-3.5" style={{ color: textMuted }} />
        <span className="text-sm" style={{ color: textMuted }}>Account Intelligence</span>
      </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm font-heading" style={{ color: textPrimary }}>
              {filteredRetailers.length} Account Hubs Mapped · {region}
            </h3>
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px]" style={{ color: textSecondary }}>{customer.currentFormat} · {customer.location}</div>
      </div>
      <div className="hidden sm:flex items-center gap-3 flex-shrink-0 flex-wrap justify-end">
        <div className="text-right">
          ...
          <div className="text-green-600 text-xs font-bold">{customer.expansionValue}</div>
        </div>
        <div className="text-right">
        ...
        <div className="flex items-start justify-between p-6" style={{ borderBottom: `1px solid ${dividerColor}` }}>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs mt-0.5" style={{ color: textSecondary }}>{customer.role} · {customer.location}</div>
          ...
          {!scriptView ? (
            <>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl space-y-2" style={{ background: surfaceBg, border: surfaceBorder }}>
                  <div className="flex items-center gap-2 mb-2">
                    <Package className="w-3.5 h-3.5" style={{ color: textSecondary }} />
                    <span className="text-[10px] uppercase tracking-wider" style={{ color: textSecondary }}>Current Format</span>
                  </div>
                  <div className="font-semibold text-sm" style={{ color: textPrimary }}>{customer.currentFormat}</div>
                  <div className="text-xs" style={{ color: textSecondary }}>{customer.currentSpec}</div>
                  <div className="flex items-center gap-1.5 mt-2">
                    <DollarSign className="w-3 h-3 text-green-600" />
                    <span className="text-green-600 text-xs font-bold">{customer.expansionValue} expansion value</span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: "Role", value: customer.role.split(" ").slice(0, 2).join(" "), icon: Briefcase },
                ].map((item) => (
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-green-600 text-sm font-semibold">Account Outreach Script Generated</span>
              </div>
            ...
            <>
              ...
              >
                {generatingScript ? (
                  ...
                  <><Sparkles className="w-4 h-4" />Generate Account Outreach Script</>
                )}
              </button>
