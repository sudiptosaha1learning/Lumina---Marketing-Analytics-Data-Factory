...
export function RetailerDrillDown({ mission, region, simMultipliers, onBack }: RetailerDrillDownProps) {
  ...
  const handleGenerateScript = (customer: CustomerProfile) => {
    ...
    setTimeout(() => {
      setScriptGenerated(
        `KEY ACCOUNT TALK TRACK — ${customer.name} — ${new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}\n\n` +
        `OPENING:\n"Good [morning/afternoon], ${customer.name}. This is [Account Manager Name] from ${selectedRetailer?.name}, calling on behalf of your dedicated Vantage Packaging Key Account team. I'm reaching out because we've identified a very specific opportunity that I believe is exceptionally well-timed for you."\n\n` +
        `PERSONALISATION HOOK:\nReference: ${customer.currentVehicle} (${customer.currentVariant}) — contract position: ${customer.buybackEquity}\n\n` +
        `VALUE PROPOSITION:\n"Your current ${customer.currentVehicle} is performing exceptionally against today's cost and sustainability targets — our specialists have identified a ${customer.buybackEquity} margin advantage that, combined with our current mono-material transition priority allocation programme, creates a genuinely compelling case that may not be available beyond next quarter."\n\n` +
        `MISSION CONTEXT:\n${mission.title}\n"${mission.tagline}"\n\n` +
        `RECOMMENDED NEXT STEP:\n${customer.recommendedAction}\n\n` +
        `CONTACT PREFERENCE:\nBest window: ${customer.contactWindow}\nPreferred channel: ${customer.preferredChannel}\n\n` +
        `OBJECTION HANDLING:\n• If 'not the right time': "I completely understand. The pricing position I mentioned is time-limited — our programme closes ${closeDate(mission.therefore.timelineDaysFromNow.close)}. Would it help if I arranged a no-obligation trial run at a time that suits?"\n• If 'happy with current program': "That speaks to your excellent supplier discipline. This programme is specifically designed for our most valued accounts who are already satisfied — it's about maximising the commercial advantage your current contract has built."\n\nCLOSING:\n"Given your tenure of ${customer.tenure} with Vantage Packaging, you'll have first access. I'll follow this up with a personalised proposal document — what email address should I use?"\n\n— Generated by Lumina Intelligence Engine v3.4`
      );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <ChevronRight className="w-3.5 h-3.5" style={{ color: textMuted }} />
        <span className="text-sm" style={{ color: textMuted }}>Plant & Account Intelligence</span>
      </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold text-sm font-heading" style={{ color: textPrimary }}>
              {filteredRetailers.length} Plants & Accounts Mapped · {region}
            </h3>
          </div>
    ...
    >
      ...
      <div className="hidden sm:flex items-center gap-3 flex-shrink-0 flex-wrap justify-end">
        <div className="text-right">
          <div className="text-[9px]" style={{ color: textMuted }}>Margin</div>
          ...
          {!scriptView ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl space-y-2" style={{ background: surfaceBg, border: surfaceBorder }}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] uppercase tracking-wider" style={{ color: textSecondary }}>Current Program</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <span className="text-green-600 text-xs font-bold">{customer.buybackEquity} margin</span>
                  </div>
                </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-green-600 text-sm font-semibold">Key Account Talk Track Generated</span>
              </div>
        ...
        <div className="p-5 flex gap-3" style={{ borderTop: `1px solid ${dividerColor}` }}>
          {!scriptView ? (
            <>
              ...
              >
                {generatingScript ? (
                  <><RefreshCw className="w-4 h-4 animate-spin" />Generating Talk Track...</>
                ) : (
                  <><Sparkles className="w-4 h-4" />Generate Key Account Talk Track</>
                )}
              </button>
