...
export interface SimParams {
  revenueWeight: number;       // 0–100  — how much to weight revenue vs. volume
  marginProtection: number;    // 0–100  — incentive constraint level
  evFocus: number;             // 0–100  — EV-ready targeting intensity
  carModelMix: number;         // 0–100  — 0=Range Rover focus, 100=balanced mix
  lifecycleBias: number;       // 0–100  — 0=Acquisition, 100=Retention
  incentiveAggression: number; // 0–100  — 0=Conservative, 100=Aggressive
}
...
// Derive multipliers from SimParams + selected objectives + mission source models
function computeSimMultipliers(
  ...
  // Revenue multiplier: driven by revenueWeight and marginProtection
  const revenueMultiplier = 0.85 + (p.revenueWeight / 100) * 0.40;
  // Customers: lifecycleBias shifts volume — retention shrinks target pool but raises conversion
  const customerMultiplier = 0.70 + ((100 - p.lifecycleBias) / 100) * 0.55 + (p.evFocus / 100) * 0.20;
  // Conversion: margin protection and lower incentive aggression improve quality
  const conversionBase = 0.90 + (p.marginProtection / 100) * 0.20;
  const conversionMultiplier = conversionBase + (p.incentiveAggression / 100) * 0.08 - ((100 - p.incentiveAggression) / 100) * 0.04;

  return {
    revenue:    revenueMultiplier * affinityBoost,
    customers:  customerMultiplier,
    conversion: Math.max(0.80, Math.min(1.35, conversionMultiplier)),
  };
}
...
export function MissionCommand({ missions, region, onViewRetailers }: MissionCommandProps) {
  ...
  const handleParamChange = useCallback((key: keyof SimParams, val: number) => {
    ...
    // If already simulating, changes apply live
  }, []);
...
function BusinessDrivenPanel({
  ...
  // Which sliders are active given selected objective, fall back to all if none selected
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: panelBg, border: panelBorder }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr]" style={{ borderBottom: `1px solid ${divider}` }}>
        ...
        <div className="p-4 space-y-2" style={{ borderRight: `1px solid ${divider}` }}>
            ...
            return (
              ...
              >
                <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0"
                  style={{
                    background: isSelected ? `rgba(${hexToRgb(obj.color)}, 0.22)` : isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.06)",
                  }}>
