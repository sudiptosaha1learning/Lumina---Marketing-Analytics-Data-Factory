...
import {
  ...
  Sliders, Play, RefreshCw, ChevronDown, BadgeCheck,
...
interface ObjectiveDef {
  ...
  border: string;
  description: string;
  // multipliers applied when this objective is PRIMARY
  primary: { revenue: number; customers: number; conversion: number };
  // multipliers applied when SECONDARY
  secondary: { revenue: number; customers: number; conversion: number };
  // which sourceModel tags this objective amplifies
  affinity: string[];
}
...
function formatRevenueSim(original: string, multiplier: number): string {
  ...
  return `${prefix}${val.toFixed(2)}M`;
}

function parseConvRate(val: string): number {
  if (val === "N/A") return 0;
...
function formatConvSim(original: string, multiplier: number): string {
  if (original === "N/A") return "N/A";
  const base = parseConvRate(original);
  const val = Math.min(99, base * multiplier);
...
function computeSimMultipliers(
  ...
  // Affinity bonus: if this mission's source models match the objective's affinity, boost by 8%
  const hasAffinity = missionSourceModels.some((m) => pDef.affinity.includes(m));
  const affinityBoost = hasAffinity ? 1.08 : 1.0;
  ...
  if (sDef) {
    r  *= sDef.secondary.revenue;
    c  *= sDef.secondary.customers;
    cv *= sDef.secondary.conversion;
  }
      ...
      <div className="p-5 space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs" style={{ color: textSecondary }}>
              Select your lead business objective — this carries the highest simulation weight
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
            {OBJECTIVES.map((obj) => {
              ...
              return (
                <button
                  key={obj.id}
                  onClick={() => onSelectPrimary(obj.id)}
                ...
                >
                  <div className="flex items-center justify-between w-full">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center"
                      style={{ background: isSelected ? `rgba(${hexToRgbStr(obj.color)},0.25)` : isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.06)" }}>
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs" style={{ color: textSecondary }}>
              Optional — a supporting objective applied at reduced weight alongside the primary
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
            {OBJECTIVES.filter((o) => o.id !== primaryObjective).map((obj) => {
              const Icon = obj.icon;
              const isSelected = secondaryObjective === obj.id;
              const disabled = !primaryObjective;
              return (
                <button
                  key={obj.id}
                  onClick={() => !disabled && onSelectSecondary(obj.id)}
                  disabled={disabled}
                  className="flex flex-col items-start gap-2 p-3 rounded-xl text-left transition-all duration-200 hover:scale-[1.02] disabled:opacity-40 disabled:cursor-not-allowed"
                  style={{
                    background: isSelected ? obj.bg : surfaceBg,
                    border: `1px solid ${isSelected ? obj.border : isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)"}`,
                    boxShadow: isSelected ? `0 0 16px ${obj.bg}` : "none",
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {primaryObjective ? (
                <div className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full"
                  style={{
                    background: OBJECTIVES.find(o => o.id === primaryObjective)!.bg,
                    border: `1px solid ${OBJECTIVES.find(o => o.id === primaryObjective)!.border}`,
                    color: OBJECTIVES.find(o => o.id === primaryObjective)!.color,
                  }}>
                  <span className="font-bold">Primary:</span> {primaryObjective}
                </div>
              ) : (
              ...
              {secondaryObjective && (
                <>
                  ...
                  <div className="flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full"
                    style={{
                      background: OBJECTIVES.find(o => o.id === secondaryObjective)!.bg,
                      border: `1px solid ${OBJECTIVES.find(o => o.id === secondaryObjective)!.border}`,
                      color: OBJECTIVES.find(o => o.id === secondaryObjective)!.color,
                    }}>
                    <span className="font-bold">Secondary:</span> {secondaryObjective}
                  </div>
                </>
              )}
            </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button
                onClick={onRun}
                disabled={!canRun}
                className="flex items-center gap-2 px-4 py-1.5 rounded-xl text-xs font-bold transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed hover:scale-[1.02]"
                style={{
...
function MissionCard({ mission, region, priorityConfig: pc, index, isDark, simMultipliers, onExplain, onViewRetailers }: MissionCardProps) {
  ...
  // Compute simulated display values
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {[
            ...
            return (
              <div key={stat.label} className="p-2.5 rounded-xl text-center relative"
                style={{
                ...
                <div className="font-bold text-sm font-heading leading-none" style={{ color: changed ? "#8b5cf6" : textPrimary }}>{stat.value}</div>
                {changed && (
...
function hexToRgbStr(hex: string): string { return hexToRgb(hex); }
