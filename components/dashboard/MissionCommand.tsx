...
// Each rule maps a mission ID to a set of threshold conditions.
// If the condition is met, the mission is suppressed with a reason.

interface MissionRule {
  missionId: string;
  check: (p: SimParams, obj: BusinessObjective | null) => string | null; // returns suppression reason or null
}
...
export interface SuggestedMission {
  ...
  triggerLabel: string;    // short tag for what triggered it
  triggerColor: string;
...
export function MissionCommand({ missions, region, onViewRetailers, simState, onSimStateChange }: MissionCommandProps) {
  ...
  // ── Adaptive mission logic (only runs when sim is active) ──────────────────
...
function AdaptiveSummaryStrip({
  isDark, suppressedCount, suggestedCount, primaryObjective,
}: { isDark: boolean; suppressedCount: number; suggestedCount: number; primaryObjective: BusinessObjective | null }) {
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap px-4 py-3 rounded-xl"
      ...
      <span className="text-xs" style={{ color: textMuted }}>
        ...
        {suppressedCount > 0 && (
          <> · <span className="text-red-500 font-semibold">{suppressedCount} mission{suppressedCount > 1 ? "s" : ""} suppressed</span> due to parameter conflicts</>
        )}
        {suggestedCount > 0 && (
          <> · <span className="text-green-600 font-semibold">{suggestedCount} new mission{suggestedCount > 1 ? "s" : ""} suggested</span> based on your configuration</>
        )}
      </span>
      <span className="text-[10px] ml-auto" style={{ color: textMuted }}>Adjust sliders to update · Reset to restore all missions</span>
    </div>
  );
}
...
function BusinessDrivenPanel({
  ...
  suppressedCount, suggestedCount,
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-bold tabular-nums" style={{ color: def.color }}>{value}</span>
      </div>
...
function MissionCard({
  mission, region, priorityConfig: pc, index, isDark,
  simMultipliers, suppressed, suppressionReason,
  onExplain, onViewRetailers
}: MissionCardProps) {
  ...
  return (
    <div
      ...
      style={{
        background: suppressed
          ...
          : hovered ? `rgba(${hexToRgb(mission.color)}, 0.08)` : isDark ? "rgba(255,255,255,0.04)" : "rgba(255,255,255,0.92)",
        border: `1px solid ${
          suppressed
            ? isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.07)"
            : hovered ? `rgba(${hexToRgb(mission.color)}, 0.4)` : isDark ? "rgba(255,255,255,0.07)" : "rgba(0,0,0,0.08)"
        }`,
        backdropFilter: "blur(16px)",
        opacity: suppressed ? 0.45 : 1,
        boxShadow: !suppressed && hovered ? `0 0 32px rgba(${hexToRgb(mission.color)}, 0.12)` : isDark ? "none" : "0 2px 12px rgba(0,0,0,0.06)",
      }}
    ...
    >
      ...
      <div className="h-0.5 w-full" style={{
        background: suppressed ? (isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.08)") : `linear-gradient(90deg, ${mission.color}, transparent)`
      }} />

      {/* Suppressed overlay banner */}
      {suppressed && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 px-5 text-center"
          style={{ background: isDark ? "rgba(0,0,0,0.55)" : "rgba(255,255,255,0.65)", backdropFilter: "blur(4px)" }}>
          ...
          <div>
            ...
            <div className="text-[10px] leading-relaxed" style={{ color: isDark ? "rgba(255,255,255,0.55)" : "rgba(0,0,0,0.55)" }}>
              {suppressionReason}
            </div>
          </div>
          <div className="text-[9px] font-semibold uppercase tracking-wider px-2 py-1 rounded-full"
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1.5">
            <button disabled={suppressed} className="flex flex-col items-center gap-1 p-2 rounded-xl text-[10px] disabled:cursor-not-allowed"
            ...
            <button
              ...
              style={{ background: suppressed ? "transparent" : "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.35)", color: "#3b82f6" }}>
...
function SuggestedMissionCard({ suggestion, isDark }: SuggestedMissionCardProps) {
  ...
  return (
    <div
      ...
      onMouseLeave={() => setHovered(false)}
    >
      {/* Suggested badge strip */}
      <div className="flex items-center gap-2 px-4 py-2"
        style={{ background: `rgba(${hexToRgb(suggestion.color)}, 0.1)`, borderBottom: `1px solid rgba(${hexToRgb(suggestion.color)}, 0.2)` }}>
        <Sparkles className="w-3 h-3" style={{ color: suggestion.color }} />
        <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: suggestion.color }}>
          Suggested Mission
        </span>
        <div className="ml-auto flex items-center gap-1.5 px-2 py-0.5 rounded-full"
          style={{ background: `rgba(${hexToRgb(suggestion.triggerColor)}, 0.15)`, border: `1px solid rgba(${hexToRgb(suggestion.triggerColor)}, 0.3)` }}>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <p className="text-xs leading-relaxed mb-3" style={{ color: textSecondary }}>{suggestion.subtitle}</p>

        {/* Why suggested */}
        <div className="p-3 rounded-xl mb-4"
          style={{ background: `rgba(${hexToRgb(suggestion.color)}, 0.07)`, border: `1px solid rgba(${hexToRgb(suggestion.color)}, 0.18)` }}>
          <div className="flex items-start gap-2">
            <Info className="w-3 h-3 flex-shrink-0 mt-0.5" style={{ color: suggestion.color }} />
            <p className="text-[10px] leading-relaxed" style={{ color: isDark ? "rgba(255,255,255,0.6)" : "rgba(0,0,0,0.6)" }}>
              {suggestion.reason}
            </p>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {suggestion.sourceModels.map((id) => {
              ...
              return (
                <span key={id} className="px-2 py-0.5 rounded text-[10px]"
                  style={{ background: isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.05)", border: surfaceBorder, color: textSecondary }}>
                  {name}
                </span>
              );
            })}
